""" This module provides methods for estimating the models performance
...
def reconstruction_error(model,
                         ...
                         use_states=False,
                         absolut_error=False):
    """ This function calculates the reconstruction errors for a given models and data.

    :param model: The models.
    :type model: Valid RBM models
...
def log_likelihood_v(model,
                     ...
                     data,
                     beta=None):
    """ Computes the log-likelihood (LL) for a given models and visible data given its log partition function.
...
def log_likelihood_h(model,
                     ...
                     data,
                     beta=None):
    """ Computes the log-likelihood (LL) for a given models and hidden data given its log partition function.
...
def partition_function_factorize_v(model,
                                   ...
                                   status=False):
    """ Computes the true partition function for the given models by factoring over the visible units.
    ...
    :return: Log Partition function for the models.
    :rtype: float
    """
    if status is True:
...
def partition_function_factorize_h(model,
                                   ...
                                   batchsize_exponent='AUTO',
                                   status=False):
    """ Computes the true partition function for the given models by factoring over the hidden units.
...
def annealed_importance_sampling(model,
                                 ...
                                 betas=10000,
                                 status=False):
    """ Approximates the partition function for the given models using annealed importance sampling.
    ...
    # Sample the first time from the base models
...
def reverse_annealed_importance_sampling(model,
                                         ...
                                         status=False,
                                         data=None):
    """ Approximates the partition function for the given models using reverse annealed importance sampling.
    ...
    # Sample the first time from the true models models
