...
class GibbsSampler(object):
    ...
    def __init__(self, model):
        """ Initializes the sampler with the models.

        :param model: The models to sample from.
        :type model: Valid models class like BinaryBinary-RBM.
        """
        # Set the models
        if not hasattr(model, 'probability_h_given_v'):
            raise ex.ValueError("The models needs to implement the function probability_h_given_v!")
        if not hasattr(model, 'probability_v_given_h'):
            raise ex.ValueError("The models needs to implement the function probability_v_given_h!")
        if not hasattr(model, 'sample_h'):
            raise ex.ValueError("The models needs to implement the function sample_h!")
        if not hasattr(model, 'sample_v'):
            raise ex.ValueError("The models needs to implement the function sample_v!")
        self.model = model
...
class PersistentGibbsSampler(object):
    ...
    def __init__(self, model, num_chains):
        ...
        if not hasattr(model, 'input_dim'):
            raise ex.ValueError("The models needs to implement the parameter input_dim!")
        self.model = model
        ...
        if numx.isscalar(num_chains):
            ...
            raise ex.ValueError("Number of chains needs to be an integer or None.")
...
class ParallelTemperingSampler(object):
    ...
    def __init__(self,
        ...
        if not hasattr(model, 'energy'):
            raise ex.ValueError("The models needs to implement the function energy!")
        if not hasattr(model, 'input_dim'):
            raise ex.ValueError("The models needs to implement the parameter input_dim!")
